...
export default function BlogPostEditPage() {
  ...
  useEffect(() => {
    if (data?.data) {
      const post: BlogPost = data.data.data ?? data.data;
      setTitle(post.title);
  ...
  const updateMutation = useMutation({
    mutationFn: (formData: FormData) => blog.update(id, formData),
    onSuccess: () => {
      router.push('/blog');
    },
  });
  ...
  if (isLoading) {
    return (
      <DashboardLayout title="Modifier l'article">
        ...
        <div className="rounded-xl border border-border bg-card-bg p-6 shadow-sm">
          {updateMutation.isError && (
            <div className="mb-6 rounded-lg bg-red-50 p-3 text-sm text-danger">
              Erreur lors de la mise a jour de l&apos;article. Veuillez reessayer.
            </div>
          )}
